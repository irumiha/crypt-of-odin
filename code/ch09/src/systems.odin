...
ai_system :: proc(w: ^World, player: Entity) {
...
sprite_layer :: proc(w: ^World, i: i32) -> Draw_Layer {
	...
	// (the skull decals) on the floor where it belongs. There is no
	// Pickup component yet; a pickup is a collider on the Pickup layer.
	if has(w, i, .Actor) || has(w, i, .Contact_Damage) do return .Actor
	if has(w, i, .Collider) && w.colliders[i].layer == .Pickup do return .Loot
	return .Decal
}
