...
emscripten_allocator_proc :: proc(
	...
	aligned_alloc :: proc(size, alignment: int, zero_memory: bool, old_ptr: rawptr = nil, old_size := 0) -> ([]byte, mem.Allocator_Error) {
		...
		if old_ptr != nil {
			...
			old_offset = int(uintptr(old_ptr) - uintptr(original_old_ptr))
		...
		// realloc keeps the payload at its old offset from the block
		// base, but the alignment padding is recomputed from the new
		// base. If the two offsets differ (possible whenever the
		// requested alignment exceeds what malloc guarantees), slide
		// the payload to where the returned slice expects it.
		new_offset := int(aligned_ptr - uintptr(allocated_mem))
		if old_ptr != nil && new_offset != old_offset {
			intrinsics.mem_copy(aligned_mem,
			                    mem.ptr_offset((^u8)(allocated_mem), old_offset),
			                    min(old_size, size))
		}
	...
	aligned_resize :: proc(p: rawptr, old_size: int, new_size: int, new_alignment: int) -> ([]byte, mem.Allocator_Error) {
		...
		return aligned_alloc(new_size, new_alignment, true, p, old_size)
	}
