...
| `src/camera.odin` | changed | comment only: with the canvas tracking the window in device pixels, `compute_viewport` needs no web special-case — raylib keeps web screen units in CSS pixels and the framebuffer in device pixels, a HiDPI monitor by another name. (The Nim original's fixed-size-canvas shell *did* need one; its ch17 camera patch dissolves with that shell) |
