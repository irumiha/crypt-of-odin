...
Dungeon :: struct {
	...
	stairs_room: int, // stairs down (or the throne, on the last floor)
	sealed:      [dynamic][2]i32,
	dissolved:   [dynamic][2]i32, // where seals were, for relock
}
...
generate :: proc(seed: i64, floor_num: int, final := false) -> (d: Dungeon) {
	...
	// it. On the final floor the deepest room holds no stairs; it is
	// the throne room, and what waits there is the caller's business.
...
unlock :: proc(d: ^Dungeon) {
	...
	for s in d.sealed {
		...
		append(&d.dissolved, s)
	}
	clear(&d.sealed)
}

relock :: proc(d: ^Dungeon) {
	// The seals return: every dissolved door tile turns back to
	// .Sealed. The boss fight's lock-in. Undone by unlock, like any
	// other seal.
	for s in d.dissolved {
		set_tile(&d.tilemap, s.x, s.y, .Sealed)
		append(&d.sealed, s)
	}
	clear(&d.dissolved)
}
...
inside_room :: proc(d: Dungeon, room: int, pos: rl.Vector2,
                    margin_tiles: i32 = 2) -> bool {
	// Whether a world position is well inside a room's interior, at
	// least margin_tiles from its walls. The boss lock-in uses this so
	// the seals never close on top of someone still in the doorway.
	r := d.rooms[room]
	left := f32((r.gx * ROOM_W + margin_tiles) * TILE_SIZE)
	top := f32((r.gy * ROOM_H + margin_tiles) * TILE_SIZE)
	right := f32(((r.gx + 1) * ROOM_W - margin_tiles) * TILE_SIZE)
	bottom := f32(((r.gy + 1) * ROOM_H - margin_tiles) * TILE_SIZE)
