...
damage_system :: proc(w: ^World) {
	...
	// Reads: contacts, Collider, Contact_Damage. Writes: Health,
	// Velocity, damage_events (frame scratch for the UI and, later,
	// the game-feel systems).
	clear(&w.damage_events)
	for c in w.contacts {
		if alive(w, c.a) && alive(w, c.b) &&
		   ...
		   w.healths[c.b.idx].invuln <= 0 {
			...
			hit := collider_rect(w, c.b.idx)
			append(&w.damage_events, Damage_Event{
				pos    = {hit.x + hit.width / 2, hit.y - 8},
				amount = w.contact_damages[c.a.idx].amount,
			})
...
boss_system :: proc(w: ^World,
                    dt: f32,
                    allocator := context.temp_allocator) -> []rl.Vector2 {
	...
	// does it (death_system's pattern).
	// Reads: Boss, Health, Ai, Position. Writes: Boss, Ai.
	spots := make([dynamic]rl.Vector2, allocator)
	for i in query(w, {.Boss, .Health, .Ai, .Position}) {
		switch w.bosses[i].phase {
		case .Stalk:
			if w.healths[i].hp * 2 <= w.healths[i].max_hp {
				w.bosses[i].phase = .Enrage
				w.ais[i].chase_speed *= 1.6
			}
		case .Enrage:
			w.bosses[i].minion_timer -= dt
			if w.bosses[i].minion_timer <= 0 {
				w.bosses[i].minion_timer = 3.5
				append(&spots, w.positions[i])
			}
		}
	}
	return spots[:]
...
hover_system :: proc(w: ^World, point: rl.Vector2) {
	...
	w.hovered = -1
	for i in query(w, {.Position, .Collider, .Pickup}) {
		if rl.CheckCollisionPointRec(point, collider_rect(w, i)) {
			w.hovered = i
		}
	}
}
