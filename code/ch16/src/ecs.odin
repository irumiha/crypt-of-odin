...
World :: struct {
	...
	float_texts:     [dynamic]i32, // the number a damage popup shows
	bosses:          [dynamic]Boss,
	// Frame scratch, not a component: contact pairs found this frame,
	// rewritten by contact_system and read by whoever cares. Damage
	// events work the same way (damage_system writes, the UI reads).
	contacts:      [dynamic]Contact,
	damage_events: [dynamic]Damage_Event,
	hovered:       i32, // slot under the mouse cursor, or -1
}

make_world :: proc() -> World {
	// Odin zero-values every field, but "no slot hovered" is -1, so a
	// World must be made, not merely declared. The constructor has
	// waited since Chapter 4 for exactly this.
	return {hovered = -1}
}
...
destroy_world :: proc(w: ^World) {
	...
	delete(w.pickup_kinds); delete(w.float_texts); delete(w.bosses)
	delete(w.contacts); delete(w.damage_events)
}
...
spawn :: proc(w: ^World, comps: Comp_Set) -> Entity {
	...
	if len(w.free_slots) > 0 {
		...
		append(&w.float_texts, 0)
		append(&w.bosses, Boss{})
	}
	w.masks[idx] = comps
	return {idx = idx, gen = w.gens[idx]}
}
...
dump :: proc(w: ^World, e: Entity,
	...
	if .Float_Text in m {
		fmt.sbprintf(&b, "\n  text      \"%d\"", w.float_texts[e.idx])
	}
	if .Boss in m {
		fmt.sbprintf(&b, "\n  boss      %v (minion call in %.1fs)",
		             w.bosses[e.idx].phase, w.bosses[e.idx].minion_timer)
	}
	if .Player in m {
